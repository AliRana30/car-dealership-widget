(function() {
  ...
  // Use cached config dimensions if available so we don't wait for widget-ready
  if (shouldReopen) {
    try {
      const cachedCfgStr = sessionStorage.getItem('myfrontdesk_config_' + widgetId);
      if (cachedCfgStr) {
        widgetConfig = JSON.parse(cachedCfgStr);
      }
    } catch (_) {}
    setTimeout(function() {
