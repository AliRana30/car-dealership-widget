...
export function fromConfigurationRecord(record: WidgetConfigurationRecord): VoiceWidgetConfig {
  return {
    ...
    branding: {
      ...
      startLabel: record.branding?.startLabel || 'Start Conversation',
      connectingLabel: record.branding?.connectingLabel || 'Connecting...',
      connectedLabel: record.branding?.connectedLabel || "You're connected",
      endLabel: record.branding?.endLabel || 'End Call',
      retryLabel: record.branding?.retryLabel || 'Try Again',
      muteLabel: record.branding?.muteLabel || 'Mute',
      unmuteLabel: record.branding?.unmuteLabel || 'Unmute',
      errorMessage: record.branding?.errorMessage || 'Unable to connect. Please check your connection and try again.',
      callEndedMessage: record.branding?.callEndedMessage || 'The call has ended. Thank you!',
      placeholderText: record.branding?.placeholderText || record.chat?.placeholderText || 'Type your message...',
      agentMessageName: record.branding?.agentMessageName || record.chat?.agentMessageName || 'Agent',
      userMessageName: record.branding?.userMessageName || record.chat?.userMessageName || 'You',
    },
    avatar: record.branding?.avatar || (record as any).avatar || {
      enabled: false,
      src: undefined,
      fallbackText: undefined,
      size: 44,
