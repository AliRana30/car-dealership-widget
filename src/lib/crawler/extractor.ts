...
export function parseSitemapFromRobots(robotsTxt: string, baseUrl: string): string[] {
  const sitemaps: string[] = [];
  const re = /^Sitemap:\s*(.+)$/gim;
  let m;
  while ((m = re.exec(robotsTxt)) !== null) {
    try {
      new URL(m[1].trim()); // validates
...
export function extractInternalLinks(html: string, baseUrl: string, disallowed: string[]): string[] {
  const links: string[] = [];
  const base = new URL(baseUrl);
  const re = /<a[^>]*\shref=["']([^"'#?][^"']*)["'][^>]*>/gi;
  let m;
  while ((m = re.exec(html)) !== null) {
    try {
      ...
      if (url.pathname.match(/\.(pdf|png|jpg|jpeg|gif|svg|ico|css|js|woff|xml|zip)$/i)) continue;
      const path = url.pathname;
      if (disallowed.some(d => path.startsWith(d))) continue;
      // Skip admin/auth paths
      if (path.match(/\/(admin|login|logout|signup|register|cart|checkout|account|wp-admin|wp-login)/i)) continue;
